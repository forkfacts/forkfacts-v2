...
import { DeleteFavorite, FoodSearchView } from "@forkfacts/components";
...
const Favorites = () => {
  ...
  const [openEmptySearch, setEmptyOpenSearch] = useState(false);
  ...
  const handleRemoveToFavorites = async (name: string) => {
    let res = await removeFavorite(name);
    if (res.status === 200) {
      setFavourites(await fetchFavorites());
      setDeleteFavorite({
        open: !deleteFavorite.open,
        name: "",
      });
    }
  };
  ...
  const handleEmptySearch = () => {
    setEmptyOpenSearch(true);
  };

  const handleFavoriteDelete = (name: string) => {
    setDeleteFavorite({
      open: !deleteFavorite.open,
      name: name,
    });
  ...
  return (
    <Page className={!favourites.length ? "bg-surface" : "bg-md-light-surface"}>
      {openEmptySearch ? (
        <>
          <FoodSearchView
            setOpenSearch={setEmptyOpenSearch}
          ...
          {openSearch ? (
            ...
            <div className="bg-[#ebebeb]">
              <Navbar
                ...
                right={
                  <IoIosSearch className="w-[24px] h-[24px] text-main" onClick={handleFocus} />
                }
                    ...
                    <div className="p-[0px] flex-col justify-start items-center gap-6 flex -mt-8">
                      ...
                      <div className="p-[0px] flex-col justify-start items-center gap-6 flex">
                        ...
                        <div className="px-6 py-[10px] bg-indigo-100 bg-opacity-50 rounded-[36px] flex-col justify-center items-center gap-2 flex">
                          <div
                            className="text-center font-500 prose-labelLarge text-primary-40"
                          ...
                          <div className="w-full px-4 py-1 rounded-lg flex justify-between items-center gap-[56px]">
                            ...
                            <div>
                              <TfiTrash
                                ...
                                onClick={() => handleFavoriteDelete(item.name)}
                              />
                            </div>
                          </div>
      ...
      <DeleteFavorite
        ...
        onCancelFavoriteDelete={onCancelFavoriteDelete}
